...
export default function Slider({
	...
	return (
		/* eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions */
		   <div
			   className={`ma_plugin-slider ${className}`}
			   ref={sliderRef}
			   onMouseEnter={handleMouseEnter}
			   onMouseLeave={handleMouseLeave}
			   onKeyDown={handleKeyDown}
			   role="region"
			   aria-label={__('Slider', 'ma-plugin')}
			   aria-roledescription="carousel"
			   tabIndex="0"
		   >
			   <div className="ma_plugin-slider__viewport">
				   <div
					   className="ma_plugin-slider__track"
					   style={{
				   ...
				   >
					{slides.map((slide, index) => (
						   <div
							   key={slide.id || index}
							   className="ma_plugin-slider__slide"
								...
								<>
									{slide.image && (
										   <img
											   ...
											   className="ma_plugin-slider__image"
										   />
									)}
									{slide.title && (
										   <h3 className="ma_plugin-slider__title">
											{slide.title}
										</h3>
									)}
									{slide.caption && (
										   <p className="ma_plugin-slider__caption">
											{slide.caption}
										</p>
									)}
								</>
			   ...
			   {showArrows && totalSlides > slidesToShow && (
				   <>
					   <Button
						   className="ma_plugin-slider__arrow ma_plugin-slider__arrow--prev"
						   onClick={prevSlide}
						   icon={chevronLeft}
						   label={__('Previous slide', 'ma-plugin')}
						   disabled={!infinite && currentIndex === 0}
					   />
					   <Button
						   className="ma_plugin-slider__arrow ma_plugin-slider__arrow--next"
						   onClick={nextSlide}
						   icon={chevronRight}
						   label={__('Next slide', 'ma-plugin')}
			   ...
			   {showDots && totalSlides > slidesToShow && (
				   <div
					   className="ma_plugin-slider__dots"
					   role="tablist"
					   aria-label={__('Slider navigation', 'ma-plugin')}
				   >
					{Array.from({
						length:
							Math.ceil(
						...
						<button
							key={index}
							   className={`ma_plugin-slider__dot ${
								   index ===

...
export default function Edit({ attributes, setAttributes, context }) {
	...
	const postType = context.postType || 'digital-magazine';
	...
	const blockProps = useBlockProps({
		className: 'wp-block-ma-plugin-digital-magazine-collection',
	});
			...
			>
				{Array.isArray(posts) && posts.length > 0 ? (
					posts.map((post) => (
						<article
							key={post.id}
							className="wp-block-ma-plugin-digital-magazine-collection__item"
						>
							{displayFeaturedImage && post.featured_media && (
								<div className="wp-block-ma-plugin-digital-magazine-collection__image">
									<img
										src={
											post._embedded?.[
												'wp:featuredmedia'
							...
							{displayTitle && (
								<h3 className="wp-block-ma-plugin-digital-magazine-collection__title">
									{post.title?.rendered ||
							...
							{displayExcerpt && (
								<div
									className="wp-block-ma-plugin-digital-magazine-collection__excerpt"
							...
							{displayMeta && (
								<div className="wp-block-ma-plugin-digital-magazine-collection__meta">
									<span className="wp-block-ma-plugin-digital-magazine-collection__date">
										{new Date(
											post.date
										).toLocaleDateString()}
									</span>
					...
					<p className="wp-block-ma-plugin-digital-magazine-collection__placeholder">

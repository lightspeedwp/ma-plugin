...
export default function Edit({ attributes, setAttributes }) {
	...
	const blockProps = useBlockProps({
		className: 'wp-block-ma_plugin-webinar-slider',
	});
	...
	return (
		<>
			...
			<div {...blockProps}>
				{source === 'custom' && (
					<>
						<div className="wp-block-ma_plugin-webinar-slider__viewport">
							<div className="wp-block-ma_plugin-webinar-slider__track">
								{slides.length > 0 ? (
									slides.map((slide, index) => (
										<div
											key={slide.id}
											className={`wp-block-ma_plugin-webinar-slider__slide ${index === currentSlide ? 'is-active' : ''}`}
										>
											<MediaUploadCheck>
												<div
													className="wp-block-ma_plugin-webinar-slider__image-wrapper"
													onClick={() => {}}
													role="button"
													tabIndex={0}
												>
													{slide.image ? (
														...
														<div className="wp-block-ma_plugin-webinar-slider__placeholder">
															{__(
											...
											<div className="wp-block-ma_plugin-webinar-slider__slide-content">
												<TextControl
													label={__(
														'Title',
														'ma-plugin'
											...
											<Button
												...
												className="wp-block-ma_plugin-webinar-slider__remove-slide"
											>
												{__(
													'Remove Slide',
													'ma-plugin'
									...
									<div className="wp-block-ma_plugin-webinar-slider__empty">
										{__(
							...
							{slides.length > 1 && (
								<div className="wp-block-ma_plugin-webinar-slider__nav">
									{slides.map((_, index) => (
										<button
											key={index}
											className={`wp-block-ma_plugin-webinar-slider__dot ${index === currentSlide ? 'is-active' : ''}`}
											onClick={() =>
							...
							<Button
								variant="primary"
								onClick={addSlide}
								className="wp-block-ma_plugin-webinar-slider__add-slide"
							>
				...
				{source === 'posts' && (
					<div className="wp-block-ma_plugin-webinar-slider__posts-notice">
						{__(
				...
				{source === 'repeater' && (
					<div className="wp-block-ma_plugin-webinar-slider__repeater-notice">
						{__(

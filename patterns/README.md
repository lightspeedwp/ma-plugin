...
```
ma-plugin/-{pattern-purpose}
```

- `ma-plugin`: The plugin namespace (e.g., `tour-operator`)
- ``: The relevant post type (e.g., `tour`, `item`, `event`)
...
- Avoid using only `ma-plugin/ma-plugin-{pattern}` as it is not unique enough for complex plugins.
